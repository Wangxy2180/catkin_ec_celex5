...
// copy from https://github.com/uzh-rpg/rpg_corner_events/blob/master/corner_event_detector/include/corner_event_detector/timer.h
//
 ...
 */
        ...
        protected:
            ...
            auto cast_d(Args &&...args) -> decltype(std::chrono::duration_cast<unit>(std::forward<Args>(args)...))
            {
                return std::chrono::duration_cast<unit>(std::forward<Args>(args)...);
            }

            template<typename... Args>
            auto cast(Args &&...args) -> decltype(std::chrono::time_point_cast<unit>(std::forward<Args>(args)...))
            {
                return std::chrono::time_point_cast<unit>(std::forward<Args>(args)...);
            }

            auto now() -> decltype(std::declval<Timer<unit>>().cast(details::default_clock_t::now()))
            {
